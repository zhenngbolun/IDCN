...
images_dir = 'F:/dataset/DIV2K/DIV2K_JPEG/y/'
LR_dir = 'F:/dataset/DIV2K/QF'+str(QF)+'/y/'

qy = get_table(luminance_quant_table, QF)
qc = get_table(chrominance_quant_table, QF)
sigma = get_sigma_Y(QF)
...
model = IDCN_Y(64, 8, PyramidCells, 64, qy)
model.summary()
gt_list, ns_list, name_list, out_dir = get_test_list('LIVE1',QF, sigma=sigma, is_crop=False)
model_head = 'weights/IDCN_Y_'+str(QF)+'_8_64_'
temp_path = 'temp_v'+str(random.randint(100000,999999))+'.jpg'

model.load_weights(model_head+'best.h5', by_name=True)
min_loss = validate(model,gt_list, ns_list,out_dir, name_list, False)
'''
for i in range(5,20):
	print("QF: %d"%(i))
	gt_list, ns_list, name_list, out_dir = get_test_list('LIVE1',i, sigma=sigma, is_crop=False)
	validate(model,gt_list, ns_list,out_dir, name_list, False)
exit(0)
'''
...
for m in range(1,100):
	...
	for file in file_list:
		...
		tmp = cv2.imread(LR_dir+s[0]+'.jpg', cv2.IMREAD_GRAYSCALE)
		...
		if img_count % 100 == 0:			
			...
			for i in range(0, x_array.shape[0]-batch_size, batch_size):
				...
				if batches % 10000 == 0:
					...
					print ('=============', lr, batches, psnr, best_batches, min_loss, '=============',datetime.now().strftime('%H:%M:%S'))
